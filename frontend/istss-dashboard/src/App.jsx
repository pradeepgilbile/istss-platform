...
{page==="devices"&&<div>
  ...
  <div className="data-table-wrap">
    <table className="data-table">
      <thead><tr>{["Device ID","Linked Chowk","Location","Network","Status","Created","Actions"].map(h=><th key={h}>{h}</th>)}</tr></thead>
      <tbody>{devices.length===0?<tr><td colSpan={7} className="data-table-empty">No devices registered. Add one above.</td></tr>:
        devices.map(d=>{const linkedChowk=chowks.find(c=>c.id===d.chowk_id);return <tr key={d.id}>
          ...
          <td>
            <button onClick={()=>{setEditId(d.id);setForm({device_id:d.device_id,status:d.status,location:d.location||"",network:d.network||"",chowk_id:d.chowk_id?(d.chowk_id+":"+(linkedChowk?.name||"")):""});}} className="btn btn-primary btn-sm" style={{marginRight:6}}>Edit</button>
