...
def block_cte(df):
    """CTE for green-phase dedup: detects camera switches when vehicle_classification changes between heartbeats. Each block of identical classifications = one signal's green phase reading. MAX per block, SUM across blocks."""
    return f"WITH ordered AS (SELECT *,LAG(vehicle_classification::text) OVER (PARTITION BY device_id ORDER BY created_at) as prev_vc FROM istss_live_traffic WHERE {df}), blocks AS (SELECT *,SUM(CASE WHEN prev_vc IS NULL OR vehicle_classification::text!=prev_vc THEN 1 ELSE 0 END) OVER (PARTITION BY device_id ORDER BY created_at) as block_id FROM ordered)"
...
async def summary(u:dict=Depends(auth)):
    ...
    if USE_PG:
        ...
        # Pull today's live traffic totals (deduplicated by green phase block)
        _df="created_at>=CURRENT_DATE"
        live_ts=db_exec(f"{block_cte(_df)} SELECT COALESCE(SUM(mt),0) as s FROM (SELECT MAX(time_saved_seconds) as mt FROM blocks GROUP BY device_id,block_id) sub",fetch=True)
        live_co2=db_exec(f"{block_cte(_df)} SELECT COALESCE(SUM(mc),0) as s FROM (SELECT MAX(estimated_co2_kg) as mc FROM blocks GROUP BY device_id,block_id) sub",fetch=True)
...
async def traffic_summary(period:str=Query("day"),u:dict=Depends(auth)):
    """Aggregated traffic summary — deduplicated by green phase (one count per camera block). period=day|week|month"""
    ...
    try:
        ...
        # Vehicle classification — per green-phase block (pick row with MAX total per block)
        vc_data=db_exec(f"{cte} SELECT COALESCE(SUM(COALESCE((vc->>'Car')::int,0)),0) as \"Car\",COALESCE(SUM(COALESCE((vc->>'Motorcycle')::int,0)),0) as \"Motorcycle\",COALESCE(SUM(COALESCE((vc->>'Bus')::int,0)),0) as \"Bus\",COALESCE(SUM(COALESCE((vc->>'Truck')::int,0)),0) as \"Truck\",COALESCE(SUM(COALESCE((vc->>'Bicycle')::int,0)),0) as \"Bicycle\" FROM (SELECT DISTINCT ON (device_id,block_id) vehicle_classification as vc FROM blocks WHERE vehicle_classification IS NOT NULL ORDER BY device_id,block_id,total_vehicles DESC) sub",fetch=True)
        ...
        if period=="day":
            ...
            tunit="day"
        hourly=db_exec(f"{cte} SELECT t as hour,COALESCE(SUM(mv),0) as vehicles,COALESCE(SUM(mc),0) as co2 FROM (SELECT date_trunc('{tunit}',created_at) as t,device_id,block_id,MAX(total_vehicles) as mv,MAX(estimated_co2_kg) as mc FROM blocks GROUP BY date_trunc('{tunit}',created_at),device_id,block_id) sub GROUP BY t ORDER BY hour",fetch=True) or []
        trend=[{"hour":str(h.get("hour","")),"vehicles":int(h["vehicles"]),"co2":round(float(h["co2"]),2)} for h in hourly]
        # Per-chowk breakdown
        chowks_data=db_exec(f"{cte} SELECT sub.chowk_id,COALESCE(c.name,sub.chowk_id) as chowk_name,COALESCE(NULLIF(c.code,''),sub.chowk_id) as chowk_code,COALESCE(SUM(mv),0) as vehicles,COALESCE(SUM(mc),0) as co2 FROM (SELECT chowk_id,device_id,block_id,MAX(total_vehicles) as mv,MAX(estimated_co2_kg) as mc FROM blocks GROUP BY chowk_id,device_id,block_id) sub LEFT JOIN istss_chowks c ON c.id=sub.chowk_id GROUP BY sub.chowk_id,c.name,c.code",fetch=True) or []
